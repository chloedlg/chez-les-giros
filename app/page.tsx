...
type ShoppingItem = { id: string; name: string; added_by_id: string | null; is_done: boolean; created_at: string }
...
const S = {
  ...
  tabBtn: (active: boolean, accent?: string) => ({
    ...
    justifyContent: 'center', padding: '8px 2px', gap: '2px', cursor: 'pointer',
    border: 'none', background: 'none', color: active ? (accent || C.primary) : C.muted,
    fontSize: '9px', fontWeight: active ? '600' : '400', transition: 'color 0.15s',
  }),
  tabIcon: { fontSize: '20px', lineHeight: '1' },
  card: {
    backgroundColor: C.card, borderRadius: '14px', padding: '14px',
    marginBottom: '10px', border: `1px solid ${C.border}`, boxShadow: '0 1px 3px rgba(53,99,212,0.06)',
  },
  ...
  btnIcon: (color: string, bg: string) => ({
...
export default function Home() {
  ...
  useEffect(() => {
    const today = getToday()
    const subs = [
      supabase.channel('rt-dinner').on('postgres_changes', { event: '*', schema: 'public', table: 'dinner_responses' }, async () => {
        const { data } = await supabase.from('dinner_responses').select('*').eq('date', today); if (data) setDinnerResponses(data)
      }).subscribe(),
      supabase.channel('rt-chores').on('postgres_changes', { event: '*', schema: 'public', table: 'chores' }, async () => {
        const { data } = await supabase.from('chores').select('*').order('created_at'); if (data) setChores(data)
      }).subscribe(),
      supabase.channel('rt-shopping').on('postgres_changes', { event: '*', schema: 'public', table: 'shopping_items' }, async () => {
        const { data } = await supabase.from('shopping_items').select('*').order('created_at'); if (data) setShoppingItems(data)
      }).subscribe(),
      supabase.channel('rt-corse').on('postgres_changes', { event: '*', schema: 'public', table: 'corse_tasks' }, async () => {
        const { data } = await supabase.from('corse_tasks').select('*').order('sort_order'); if (data) setCorseTasks(data)
      }).subscribe(),
  ...
  async function toggleMemberActive(member: Member) { await supabase.from('members').update({ is_active: !member.is_active }).eq('id', member.id) }
    ...
    <div style={S.app}>
        ...
        <div style={S.overlay} onClick={() => setShowMembersModal(false)}>
          <div style={S.modal} onClick={e => e.stopPropagation()}>
            ...
            {members.filter(m => !m.is_mom).map(member => (
              <div key={member.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: `1px solid ${C.border}` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <div style={{
                    width: '38px', height: '38px', borderRadius: '50%',
                    backgroundColor: member.is_active ? C.primaryLight : '#F0F0F0',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: '15px', fontWeight: '700', color: member.is_active ? C.primary : '#B0B0B0',
                  }}>{member.name[0]}</div>
                  <div>
                    <div style={{ fontSize: '15px', fontWeight: '500', color: member.is_active ? C.text : '#B0B0B0' }}>{member.name}</div>
                    <div style={{ fontSize: '12px', color: member.is_active ? C.green : '#B0B0B0', fontWeight: '500' }}>
                      {member.is_active ? '✅ Actif·ve' : '⏸️ En pause'}
                    </div>
                  </div>
                </div>
                <button onClick={() => toggleMemberActive(member)} style={member.is_active ? S.btn(C.orange, C.orangeLight) : S.btn(C.green, C.greenLight)}>
                  {member.is_active ? 'Pause' : 'Réactiver'}
                </button>
              </div>
            ))}
            <button onClick={() => setShowMembersModal(false)} style={{ ...S.btnFill(C.primary), width: '100%', padding: '12px', marginTop: '20px', fontSize: '15px' }}>
              Fermer
            </button>
          </div>
        </div>
      ...
      <div style={S.header}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <button onClick={() => setShowMembersModal(true)} title="Gérer les membres" style={{
              width: '34px', height: '34px', borderRadius: '50%', backgroundColor: 'rgba(255,255,255,0.2)',
              border: '2px solid rgba(255,255,255,0.4)', color: 'white', fontSize: '15px',
            ...
            <div onClick={() => setShowIdentityModal(true)} style={{
              width: '34px', height: '34px', borderRadius: '50%', backgroundColor: 'rgba(255,255,255,0.2)',
              border: '2px solid rgba(255,255,255,0.4)', color: 'white', fontSize: '14px',
              fontWeight: '600', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
            }}>{currentMember ? currentMember.name[0] : '?'}</div>
          </div>
        </div>
      </div>
      ...
      <div style={S.content}>
        ...
        {activeTab === 'courses' && <CoursesTab items={shoppingItems} members={members} currentMember={currentMember}
          ...
          onAddItems={async names => { if (!currentMember) return; await supabase.from('shopping_items').insert(names.map(name => ({ name, added_by_id: currentMember.id }))) }}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {members.map(member => (
            <button key={member.id} onClick={() => onSelect(member)} style={{
              ...
              borderRadius: '12px', border: `2px solid ${C.border}`, backgroundColor: C.bg,
              cursor: 'pointer', textAlign: 'left', width: '100%',
            }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', backgroundColor: C.primaryLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', fontWeight: '700', color: C.primary, flexShrink: 0 }}>
                {member.name[0]}
              </div>
              <div>
      ...
      <div style={{ ...S.card, border: `2px solid ${C.primary}` }}>
        ...
        <div style={{ marginBottom: '10px' }}>
          ...
          {comingHome.length === 0 && notComing.length === 0
            ...
            : <>
              {comingHome.map(r => <div key={r.id} style={{ fontSize: '14px', color: C.text, marginBottom: '2px', display: 'flex', gap: '6px' }}>
                <span>{r.status === 'assiette' ? '🍽️' : '🟢'}</span>
                <span><strong>{getName(r.member_id)}</strong>{r.arrival_time ? ` · ${r.arrival_time}` : ''}{r.status === 'assiette' ? ' (assiette)' : ''}</span>
              </div>)}
              {notComing.map(r => <div key={r.id} style={{ fontSize: '14px', color: C.muted, marginBottom: '2px', display: 'flex', gap: '6px' }}>
                <span>🔴</span><span>{getName(r.member_id)} · absent·e</span>
              </div>)}
            </>}
        </div>
      ...
      {currentMember?.is_active && (
        <div style={S.card}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '14px' }}>
            {(Object.entries(statusConfig) as [DinnerStatus, typeof statusConfig.oui][]).map(([status, cfg]) => (
              <button key={status} onClick={() => setSelectedStatus(status)} style={{
                ...
                border: `2px solid ${selectedStatus === status ? cfg.color : C.border}`,
                backgroundColor: selectedStatus === status ? cfg.bg : C.bg,
                color: selectedStatus === status ? cfg.color : C.text,
                fontWeight: selectedStatus === status ? '600' : '400', cursor: 'pointer', fontSize: '14px',
              }}>{cfg.label}</button>
            ))}
          </div>
          {(selectedStatus === 'oui' || selectedStatus === 'assiette') && (
            <div style={{ marginBottom: '14px' }}>
              <label style={{ fontSize: '13px', color: C.muted, display: 'block', marginBottom: '6px' }}>Heure d&apos;arrivée estimée (optionnel)</label>
          ...
          <button onClick={async () => { if (!selectedStatus) return; setSaving(true); await onUpdateResponse(selectedStatus, arrivalTime || null); setSaving(false) }}
            ...
            style={{ ...S.btnFill(C.primary), width: '100%', padding: '12px', fontSize: '15px', opacity: !selectedStatus || saving ? 0.5 : 1 }}>
      ...
      {members.map(member => {
        ...
        return (
          <div key={member.id} style={{ ...S.card, display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 14px', ...(isMe ? { borderColor: C.primary, borderWidth: '2px' } : {}) }}>
            <div style={{ width: '38px', height: '38px', borderRadius: '50%', backgroundColor: cfg ? cfg.bg : '#F0F2F8', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', fontWeight: '700', color: cfg ? cfg.color : C.muted, flexShrink: 0 }}>
              {member.name[0]}
            </div>
            <div style={{ flex: 1 }}>
              ...
              {response
                ? <div style={{ fontSize: '13px', color: cfg?.color }}>{cfg?.label}{response.arrival_time && <span style={{ color: C.muted, fontWeight: '400' }}> · {response.arrival_time}</span>}</div>
    ...
    <div>
      ...
      <div style={S.card}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <input value={newChore} onChange={e => setNewChore(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && newChore.trim()) { setAdding(true); onAddChore(newChore.trim()).then(() => { setNewChore(''); setAdding(false) }) } }}
            placeholder="Ex: Passer l'aspirateur…" style={{ ...S.input, flex: 1 }} />
          <button onClick={() => { if (!newChore.trim()) return; setAdding(true); onAddChore(newChore.trim()).then(() => { setNewChore(''); setAdding(false) }) }}
            disabled={adding || !newChore.trim()} style={{ ...S.btnFill(C.green), opacity: adding || !newChore.trim() ? 0.5 : 1 }}>{adding ? '…' : 'Ajouter'}</button>
        </div>
      </div>
          ...
          <div key={chore.id} style={S.card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
              <div style={{ flex: 1 }}>
                ...
                {assignedName
                  ? <div style={{ fontSize: '13px', color: C.green, marginTop: '3px', fontWeight: '500' }}>👤 {assignedName}{isMe && <span style={{ color: C.primary }}> (toi !)</span>}</div>
              ...
              <div style={{ display: 'flex', gap: '5px', flexShrink: 0, flexWrap: 'wrap' as const, justifyContent: 'flex-end' }}>
                ...
                <button onClick={() => onDeleteChore(chore.id)} style={S.btnIcon(C.red, C.redLight)} title="Supprimer">✕</button>
              </div>
            </div>
          </div>
        )
...
// ================================================================
// ONGLET COURSES (avec photo OCR)
// ================================================================
function CoursesTab({ items, members, currentMember, onAddItem, onAddItems, onToggleItem, onDeleteDone }: {
  ...
  async function handlePhoto(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = async (ev) => {
      ...
      try {
        // Appel OCR.space (clé demo gratuite)
        ...
        // OCR échoué — on garde juste la photo
      } finally {
        setOcrLoading(false)
      }
    }
  ...
  return (
    <div>
      <div style={{ ...S.card, backgroundColor: C.purple, color: 'white', border: 'none' }}>
        ...
        <div style={{ fontSize: '26px', fontWeight: '700' }}>{pending.length} <span style={{ fontSize: '14px', opacity: 0.85 }}>article{pending.length > 1 ? 's' : ''} à acheter</span></div>
      </div>
      ...
      <div style={S.card}>
        <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
          <input value={newItem} onChange={e => setNewItem(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter' && newItem.trim()) { setAdding(true); onAddItem(newItem.trim()).then(() => { setNewItem(''); setAdding(false) }) } }}
            placeholder="Ajouter un article…" style={{ ...S.input, flex: 1 }} />
          <button onClick={() => { if (!newItem.trim()) return; setAdding(true); onAddItem(newItem.trim()).then(() => { setNewItem(''); setAdding(false) }) }}
            disabled={adding || !newItem.trim()} style={{ ...S.btnFill(C.purple), opacity: adding || !newItem.trim() ? 0.5 : 1 }}>Ajouter</button>
        </div>
        ...
        <button onClick={() => fileInputRef.current?.click()} disabled={ocrLoading}
          ...
          {ocrLoading ? '⏳ Lecture en cours…' : '📷 Prendre en photo une liste manuscrite'}
        </button>
      ...
      {ocrPreview.length > 0 && (
        <div style={{ ...S.card, border: `2px solid ${C.purple}` }}>
          ...
          <div style={{ fontSize: '12px', color: C.muted, marginBottom: '12px' }}>Décoche ceux que tu ne veux pas ajouter</div>
          {ocrPreview.map((line, i) => (
            <div key={i} onClick={() => { const s = new Set(selectedOcrItems); s.has(i) ? s.delete(i) : s.add(i); setSelectedOcrItems(s) }}
              ...
              <div style={{
                ...
                border: `2px solid ${C.purple}`, backgroundColor: selectedOcrItems.has(i) ? C.purple : 'transparent',
                display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '12px',
              }}>{selectedOcrItems.has(i) ? '✓' : ''}</div>
              <span style={{ fontSize: '14px', color: selectedOcrItems.has(i) ? C.text : '#B0B0C0', textDecoration: selectedOcrItems.has(i) ? 'none' : 'line-through' }}>{line}</span>
            </div>
          ))}
          <div style={{ display: 'flex', gap: '8px', marginTop: '14px' }}>
            <button onClick={() => { setOcrPreview([]); setPhotoPreview(null) }} style={{ ...S.btn(C.muted, '#F0F2F8'), flex: 1, padding: '10px', textAlign: 'center' }}>Annuler</button>
            <button onClick={confirmOcrItems} disabled={selectedOcrItems.size === 0}
              style={{ ...S.btnFill(C.purple), flex: 2, padding: '10px', opacity: selectedOcrItems.size === 0 ? 0.5 : 1 }}>
      ...
      {/* Photo sans OCR */}
      {photoPreview && ocrPreview.length === 0 && !ocrLoading && (
        <div style={S.card}>
          <div style={{ fontSize: '13px', color: C.muted, marginBottom: '8px' }}>Photo de référence</div>
          <img src={photoPreview} alt="liste" style={{ width: '100%', borderRadius: '8px', maxHeight: '300px', objectFit: 'contain' }} />
          <button onClick={() => setPhotoPreview(null)} style={{ ...S.btn(C.muted, '#F0F2F8'), marginTop: '10px', width: '100%', padding: '8px', textAlign: 'center' }}>Fermer</button>
        </div>
      )}
      ...
      {done.length > 0 && <>
        ...
        {done.map(item => (
          <div key={item.id} style={{ ...S.card, display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 14px', opacity: 0.5 }}>
            ...
            <div style={{ flex: 1, textDecoration: 'line-through', color: C.muted, fontSize: '15px' }}>{item.name}</div>
          </div>
        ))}
      </>}
    </div>
...
function CorseTab({ tasks, onToggle, onAdd, onDelete, onReset }: {
  ...
  function TaskSection({ category, label, icon, color, bg, newVal, setNew }: {
    category: 'ouvrir' | 'fermer'; label: string; icon: string; color: string; bg: string
    newVal: string; setNew: (v: string) => void
  }) {
    ...
    return (
      <div style={{ ...S.card, marginBottom: '12px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
          ...
          {doneCount > 0 && (
            <button onClick={() => onReset(category)} style={S.btn(C.muted, '#F0F2F8')} title="Tout décocher">
              ↺ Réinitialiser
            </button>
          )}
        </div>
        ...
        <div style={{ height: '6px', backgroundColor: '#F0F2F8', borderRadius: '3px', marginBottom: '14px', overflow: 'hidden' }}>
        ...
        {list.map(task => (
          <div key={task.id} onClick={() => onToggle(task.id, task.is_done)}
            style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 0', borderBottom: `1px solid ${C.border}`, cursor: 'pointer' }}>
            <div style={{
              width: '22px', height: '22px', borderRadius: '6px', flexShrink: 0,
              border: `2px solid ${task.is_done ? color : C.border}`,
              backgroundColor: task.is_done ? color : 'transparent',
              display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '12px',
            }}>{task.is_done ? '✓' : ''}</div>
            ...
            <button onClick={e => { e.stopPropagation(); onDelete(task.id) }} style={{ ...S.btnIcon(C.red, 'transparent'), padding: '2px 6px', opacity: 0.5 }}>✕</button>
          </div>
        ))}
        ...
        <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
          <input value={newVal} onChange={e => setNew(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter' && newVal.trim()) { onAdd(newVal.trim(), category).then(() => setNew('')) } }}
            placeholder="Ajouter une étape…" style={{ ...S.input, flex: 1, fontSize: '13px', padding: '8px 10px' }} />
          <button onClick={() => { if (!newVal.trim()) return; onAdd(newVal.trim(), category).then(() => setNew('')) }}
            disabled={!newVal.trim()} style={{ ...S.btnFill(color), opacity: !newVal.trim() ? 0.5 : 1, padding: '8px 12px' }}>+</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div>
      <div style={{ ...S.card, backgroundImage: 'linear-gradient(135deg, #1E90C8, #2BC4A0)', color: 'white', border: 'none', textAlign: 'center', padding: '20px' }}>
      ...
      <TaskSection category="ouvrir" label="Ouvrir la maison" icon="🔑" color="#1E90C8" bg="#E3F4FB" newVal={newOuvrir} setNew={setNewOuvrir} />
      <TaskSection category="fermer" label="Fermer la maison" icon="🔒" color="#2BC4A0" bg="#E3F9F5" newVal={newFermer} setNew={setNewFermer} />
    </div>
  )
}
...
function PoubellsTab() {
  ...
  return (
    <div>
      {yellow && <div style={{ ...S.card, backgroundColor: '#F39C12', color: 'white', border: 'none', textAlign: 'center', padding: '28px 20px', marginBottom: '10px' }}>
      ...
      {brown && <div style={{ ...S.card, backgroundColor: '#795548', color: 'white', border: 'none', textAlign: 'center', padding: '28px 20px', marginBottom: '10px' }}>
        <div style={{ fontSize: '48px', marginBottom: '8px' }}>🟤</div>
      ...
      {!today && <div style={{ ...S.card, textAlign: 'center', padding: '28px 20px' }}>
        <div style={{ fontSize: '44px', marginBottom: '10px' }}>🗑️</div>
        <div style={{ fontSize: '17px', fontWeight: '600', color: C.text, marginBottom: '4px' }}>Pas de poubelle aujourd'hui</div>
        <div style={{ fontSize: '13px', color: C.muted }}>Profitez-en !</div>
      </div>}
      <div style={{ ...S.card, marginTop: today ? '0' : '8px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', backgroundColor: '#FFFDE7', borderRadius: '10px', border: '1px solid #F9A825' }}>
            ...
            <div><div style={{ fontSize: '14px', fontWeight: '600', color: C.text }}>Poubelle jaune</div>
              <div style={{ fontSize: '13px', color: C.muted }}>Mercredi · {yellow ? "aujourd'hui !" : `dans ${daysToWed}j`}</div></div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', backgroundColor: '#EFEBE9', borderRadius: '10px', border: '1px solid #A1887F' }}>
            <span style={{ fontSize: '28px' }}>🟤</span>
            <div><div style={{ fontSize: '14px', fontWeight: '600', color: C.text }}>Poubelle marron</div>
              <div style={{ fontSize: '13px', color: C.muted }}>Mardi & vendredi · {brown ? "aujourd'hui !" : `prochain : ${nextBrown.label} dans ${nextBrown.days}j`}</div></div>
          </div>
        </div>
      </div>
    </div>
